...
(()=>{
 ...
 g.eliteHandlers.signalJammer=function(e,dt){e.attackTimer-=dt;if(e.attackTimer>0||e.x>.56)return;e.attackTimer=C.enemies.jamInterval;const targets=liveCars().flatMap(x=>x.c.equipment).filter(eq=>!(eq.jam09>0));const eq=choose(targets);if(eq){eq.jam09=C.enemies.jamSeconds;g.toast(`${(eq.kind==='turret'?D.TURRETS:D.MODULES)[eq.type].name} · 신호 교란 ${C.enemies.jamSeconds}초`);}};
