...
(()=>{
 ...
 const flavor={
  ...
  skills:{
   ...
   rival:'“저 둘을 같은 칸에 두지 마.” — 배치표 가장자리의 붉은 메모. 함께 있으면 서로의 실수만 보느라 자기 일까지 망친다.',
   lover:'“이상하게 저 사람 옆에 있으면 조금 덜 무섭습니다.” 전투기록에는 이유가 적혀 있지 않지만, 둘이 같은 객차에 배치된 날의 성과는 대체로 좋았다.'
  }
 };
 function flavorBlock(text,extra=''){return text?`<blockquote class="codex-flavor${extra?' '+extra:''}">${esc(text)}</blockquote>`:'';}
 function gear(e){
  const d=(e.kind==='turret'?D.TURRETS:D.MODULES)[e.type];
  let html=(d.ancient?'<div class="ancient-gear-mark"><b>ANCIENT EQUIPMENT</b><span>고대 장비</span></div>':'')+g.equipmentHTML({...e,level:1,heat:0},null);
  html+=flavorBlock(d.flavor||flavor.gear[e.type],d.ancient?'ancient-flavor':'');
  return html;
 }
 ...
 function reward(r={}){const names={stationVisit:'정비소 이용',firstUpgrade:'무료 1차 강화 횟수 (Lv.1 → 2)',survivorSupplies:'구조 답례',skillReward:'이벤트 스킬',money:'돈',scrap:'고철',relics:'고대 잔해',distance:'타이탄 거리 (km)',moneyLoss:'돈 손실',carDamage:'객차 HP 손실',crewDamage:'직원 HP 손실',actorDamage:'출전 직원 HP 손실',engineDamage:'기관실 HP 손실',carCount:'피해 객차 수',crewCount:'피해 직원 수',gear:'장비 종류',candidates:'직원 후보 수',repairRatio:'최대 객차 HP 회복 비율',healRatio:'최대 직원 HP 회복 비율',healCrew:'직원 HP 회복',temporaryEngine:'다음 전투 엔진 속도 배율',shop:'거래소',shopFactor:'상점 가격 배율',freeItem:'상품 1개 무료',disableGear:'무작위 장비 다음 전투 정지',routeInfo:'다음 경로 정보',buff:'런 한정 무작위 강화',debuff:'런 한정 무작위 약화',medicalRescue:'의무병 구조',possibleCandidate:'추가 직원 후보',possibleRareGear:'희귀 장비',possibleTalent:'전투 재능',labReward:'실험실 무작위 보상',forceEventSkill:'강제 이벤트 스킬',allCarMaxHpDamage:'모든 객차 최대 HP 비례 피해',allCrewDamage:'모든 직원 HP 피해',blackoutNext:'다음 전투 객차 암전',destroyNonEngineCar:'비기관실 객차 파괴',disableDestroyedCarGear:'파괴 객차 장비 임시 고장',engineDebuffBattles:'엔진 출력 저하 전투 수',engineDebuff:'엔진 속도 배율',pairDamage:'관계 당사자 HP 피해',pairRelation:'관계 이벤트 스킬',pairBreakup:'관계 파탄',forceEliteNext:'다음 일반전 정예화',ancientCoreBoost:'고대 장비 효율 배율',coreRiskBattles:'보호계통 불안정 전투 수',coreRiskDamage:'객차 받는 피해 배율',breakAncientGear:'고대 장비 파손',enginePowerCapLoss:'기관실 최대 출력 감소'};
 ...
 function loreCards(){
  ...
  return `<section class="lore-progress"><div><span>도감 복원율</span><strong>${progress.percent}%</strong><small>${progress.opened} / ${progress.total} 개방</small></div><div class="lore-progress-track"><i style="width:${progress.percent}%"></i></div><p>도감 기록 ${Math.min(11,Math.floor(step/10)+1)} / 11 해금 · 종말 기록 ${Math.max(0,Math.min(11,highest+1))} / 11 해금</p></section><section class="lore-section"><header><div><span class="section-kicker">ARCHIVE RECOVERY</span><h3>도감 복원 기록</h3></div><small>0% 프롤로그는 즉시 복원되며, 이후 도감 전체 개방률이 10% 오를 때마다 새 기록이 복원됩니다.</small></header><div class="lore-grid">${progressCards}</div></section><section class="lore-section"><header><div><span class="section-kicker">APOCALYPSE LOG</span><h3>종말 기록</h3></div><small>각 종말 단계를 클리어하면 해당 기록이 해금됩니다.</small></header><div class="lore-grid">${doomCards}</div></section>`;
 }
 ...
 function attachDragScroll(el){
  let down=false,startY=0,startScroll=0,moved=false,pointerId=null;
  const reset=e=>{if(!down)return;down=false;moved=false;el.classList.remove('dragging');try{if(pointerId!==null&&el.hasPointerCapture?.(pointerId))el.releasePointerCapture(pointerId);}catch{}pointerId=null;};
  el.addEventListener('pointerdown',e=>{if(e.pointerType!=='mouse'||e.button!==0)return;down=true;moved=false;pointerId=e.pointerId;startY=e.clientY;startScroll=el.scrollTop;el.classList.add('dragging');try{el.setPointerCapture?.(e.pointerId);}catch{}});
  ...
  el.addEventListener('pointerup',reset);el.addEventListener('pointercancel',reset);el.addEventListener('lostpointercapture',reset);window.addEventListener('pointerup',reset,true);window.addEventListener('pointercancel',reset,true);window.addEventListener('blur',reset,true);
 }
