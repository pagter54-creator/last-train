...
(() => {
  ...
  function bossOffset(w){const b=g.state?.battle;if(!b?.rework||!b.arrivalLeft)return 0;const progress=clamp(1-b.arrivalLeft/C.arrivalSeconds),eased=1-Math.pow(1-progress,C.arrivalExponent);return w*C.arrivalOffscreenWidths*(1-eased);}
  function bossAlpha(){const b=g.state?.battle;if(!b?.rework||!b.defeated)return 1;return clamp(b.defeatFadeLeft/C.defeatFadeSeconds);}
  function home(p,w,h){return{x:w*p.x+bossOffset(w),y:h*p.y,scale:1,r:Math.max(23,Math.min(38,w*.033))};}
  ...
  g.drawBoss=function(ctx,w,h){const b=this.state.battle;if(!b.rework)return old(ctx,w,h);const t=A.visualClock,parts=b.parts,core=parts.find(p=>p.victory),cp=A.project(core,w,h),arachne=b.bossId==='arachne',ox=bossOffset(w);ctx.save();ctx.globalAlpha=bossAlpha();
    if(!arachne){
      // Long tracked hull, stern exhausts, recessed bay and oversized forward cannon.
      const x=w*.18+ox,y=h*.36,bw=w*.7,bh=h*.2;
    ...
    const repairCycle=b.repairCycleLeft>0;plate(ctx,cp.x-29,cp.y-41,58,82,repairCycle?'#24433d':'#1a3036');ctx.save();if(b.coreLeft>0){ctx.shadowColor='#ffd381';ctx.shadowBlur=25+Math.sin(t*12)*9;ctx.fillStyle='#ffe2a0';ctx.fillRect(cp.x-12,cp.y-31,24,62);for(let i=0;i<5;i++){const v=(t*.65+i*.2)%1;ctx.fillStyle=`rgba(202,224,206,${(1-v)*.5})`;ctx.beginPath();ctx.arc(cp.x+Math.sin(i*3)*25,cp.y-35-v*70,4+v*16,0,Math.PI*2);ctx.fill();}}else if(repairCycle){ctx.shadowColor='#79e7bc';ctx.shadowBlur=18+Math.sin(t*9)*6;ctx.fillStyle='#8ce4bd';ctx.fillRect(cp.x-8,cp.y-27,16,54);}ctx.restore();const spread=b.coreLeft>0?21:0;plate(ctx,cp.x-25-spread,cp.y-34,24,68,repairCycle?'#567b6d':'#62736c');plate(ctx,cp.x+1+spread,cp.y-34,24,68,repairCycle?'#567b6d':'#62736c');
    if(repairCycle){for(const p of parts.filter(p=>!p.victory)){const q=A.project(p,w,h),pulse=(t*.75+(p.x||0))%1;beam(ctx,cp,q,'#79e7bc',2);ctx.fillStyle='#d8ffe9';ctx.beginPath();ctx.arc(cp.x+(q.x-cp.x)*pulse,cp.y+(q.y-cp.y)*pulse,4+Math.sin(t*8+pulse)*1.5,0,Math.PI*2);ctx.fill();}}
    for(const p of parts){const q=A.project(p,w,h),color=C.grips[p.type]?.color||'#efd08a';if(!p.victory){ctx.fillStyle='#142629';ctx.fillRect(q.x-34,q.y-40,68,5);ctx.fillStyle=p.destroyed?'#999f99':'#ecbd76';ctx.fillRect(q.x-34,q.y-40,68*clamp(p.destroyed?1-p.repairLeft/p.regen:p.hp/p.maxHp),5);}text(ctx,p.victory?(b.coreLeft>0?'CORE OPEN':repairCycle?'CORE REPAIR':'CORE CLOSED'):p.name,q.x,q.y-47,p.victory&&(b.coreLeft>0||repairCycle)?'#98edbc':color);
    ...
    if(b.noticeLeft>0||b.coreLeft>0||repairCycle){const label=b.noticeLeft>0?b.notice:b.coreLeft>0?`CORE OPEN · ${Math.ceil(b.coreLeft)}초`:`REPAIR CYCLE · ${Math.ceil(b.repairCycleLeft)}초`;ctx.fillStyle='#11292be0';ctx.fillRect(w*.03,h*.65,w*.94,30);text(ctx,label,w/2,h*.65+20,b.coreLeft>0||repairCycle?'#a9f5c4':'#ffcd91',Math.max(10,Math.min(14,w/70)));}
    ctx.restore();
  };
})();
