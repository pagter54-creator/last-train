...
(() => {
  ...
  g.advanceStage=function(){if(travel||!this.state)return;A.cancelSelection();overlay.classList.remove('show');travel={run:this.state,phase:'exit',elapsed:0};this.mode='transition';this.sceneTransition={offset:0,boost:1,phase:'exit'};document.body.classList.add('route-travel');shake();this.playSound('engine');};
  g.update=function(dt){if(!travel)return update(dt);if(this.state!==travel.run){reset();return update(dt);}if(document.hidden)return;
    ...
    if(tr.phase==='exit'){const p=Math.min(1,tr.elapsed/B.transition.exitSeconds);this.sceneTransition.offset=low?0:MOVEMENT_CONFIG.width*p*p;this.sceneTransition.boost=1+B.transition.backdropBoost*p;
      if(p===1){advance();tr.targetMode=this.mode;tr.overlay=overlay.classList.contains('show');overlay.classList.remove('show');this.mode='transition';tr.phase='enter';tr.elapsed=0;this.sceneTransition={offset:low?0:MOVEMENT_CONFIG.width,boost:1+B.transition.backdropBoost,phase:'enter'};}
