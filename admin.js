(()=>{
 const g=lastRail,D=GAME_DATA,B=D.BALANCE,C={code:'9789',grant:1000,speeds:[1,2,3]};
 ...
 function consolePanel(){
  ...
  dialog.querySelectorAll('[data-distance]').forEach(b=>b.onclick=()=>{const delta=Number(b.dataset.distance)||0,max=B.run.maxTitanDistance??Infinity;g.state.titanDistance=Math.max(0,Math.min(max,g.state.titanDistance+delta));refresh();});
 ...
 const style=document.createElement('style');style.textContent='.admin-dialog{width:min(560px,90vw);max-height:85vh;overflow:auto;background:#142a30;color:#e5eee5;border:1px solid #91a996;border-radius:10px;padding:24px}.admin-dialog::backdrop{background:#061014aa}.admin-dialog button,.admin-dialog input,.admin-dialog select{background:#28434a;color:#eef4ea;border:1px solid #79938b;padding:10px;margin:5px;border-radius:5px}.admin-dialog label{display:block;margin:12px 0}.admin-actions{display:flex;flex-wrap:wrap}.admin-distance{margin:14px 0;padding:12px;border:1px solid #56706b;background:#102327}.admin-distance>b{display:block;margin-bottom:7px;color:#e4d19d}.admin-distance>div{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.admin-distance [data-distance-value]{min-width:92px;text-align:center;font-weight:bold;color:#d9e8e2}.admin-dialog [data-error]{color:#ff8b85}';document.head.append(style);
})();
