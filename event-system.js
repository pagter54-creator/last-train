...
(()=>{
  ...
  function render(){
    ...
    if(session.phase==='choices'){
      ...
      for(const c of ev().choices.filter(allowed)){const maybe=[c.reward,...(c.outcomes||[]).map(o=>o.reward)],noGear=maybe.some(r=>r.gear||r.possibleRareGear||r.labReward)&&!g.state.cars.some((_,i)=>capacity('equipment',i)),hidePreview=!!ev().hideChoicePreview;button(body,`<div><b>${esc(c.label)}${c.dispatch?' · 직원 선택':''}${c.selectTurret?' · 포탑 선택':''}</b><p class="${eventTimeCost(c)||Object.keys(c.cost||{}).length?'negative':''}">${costText(c)}</p>${hidePreview?'':`<p>${c.reward.part09?'결과는 담당 직원의 능력과 사건 설명을 확인하세요.':chanceText(c)}</p><p>${rewardHTML(c.reward)}</p>`}${noGear?'<p class="negative">장비 빈자리 없음</p>':''}${!canChoose09(c)?'<p>이 선택을 실행할 대상 또는 자원이 없습니다.</p>':''}</div>`,()=>c.dispatch?transaction(()=>{session.phase='dispatch';session.selected=c.id;}):c.selectTurret?transaction(()=>{session.phase='selectTurret';session.selected=c.id;}):commit(c),!g.canPay(c.cost)||!canChoose09(c)||(c.dispatch&&!dispatchStaff(c).length));}
    }else if(session.phase==='selectTurret'){
      const c=definition(session.selected);body.innerHTML='<p>미치광이에게 보여줄 포탑 하나를 선택하세요.</p>';
