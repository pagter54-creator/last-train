...
(()=>{
  ...
  function prepareReward(raw,seed){
    const r=rng(seed),out={...raw};if(out.survivorSupplies)Object.assign(out,pick(r,C.survivorSupplies));for(const k of ['money','scrap','relics','moneyLoss','crewDamage','actorDamage','carDamage'])if(k in out)out[k]=integer(r,out[k]);
  ...
  function render(){
    ...
    }else if(session.phase==='finish'){body.innerHTML=logsHTML();button(body,'출발 →',()=>transaction(()=>session.phase='leaving'));}
  }
