...
(()=>{
 ...
 const update=g.update.bind(g);g.update=function(dt){const result=update(dt);if(this.mode==='arrival'&&this.stationArrivalKind==='station'){const p=this.stationArrivalProgress||0;station.style.translate=`${low()?0:C.width*V.offscreenWidths*Math.pow(1-p,V.arrivalExponent)}px 0`;}else if(this.mode==='station')station.style.translate='0px 0';
