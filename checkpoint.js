...
(()=>{
 ...
 g.showSaveWarning=()=>{
  ...
  el.innerHTML=`<div class="save-guide-heading"><small>LOCAL SAVE · CLOUD SHARE · BACKUP</small><h2>저장 및 백업 안내</h2><p>LAST RAIL은 진행 상황을 <strong>현재 브라우저</strong>에 자동 저장합니다. 1.0.2부터는 세이브 관리에서 <strong>공유 코드</strong>를 만들어 다른 브라우저·기기 또는 다른 플레이어에게 저장 복사본을 전달할 수 있습니다.</p></div><div class="save-guide-grid"><article><b>자동 저장</b><p>영구 성장과 설정은 자동 저장됩니다. 진행 중인 런은 가장 최근 <strong>스테이지 시작 체크포인트</strong>에서 이어지며, 전투 도중의 순간 상태는 저장하지 않습니다.</p></article><article class="save-guide-cloud-card"><b>클라우드 공유 코드</b><p><strong>세이브 관리</strong>에서 현재 저장을 서버에 복사하고 <strong>XXXXX-XXXXX</strong> 코드를 발급할 수 있습니다. 코드는 자동 동기화 계정이 아니라 생성 시점의 복사본입니다.</p></article><article><b>JSON 파일 백업</b><p>인터넷과 무관하게 보관할 수 있는 수동 백업입니다. 큰 업데이트 전이나 장기 보관이 필요할 때 파일 내보내기를 함께 사용하세요.</p></article></div><section class="save-guide-section"><h3>브라우저를 바꾸거나 기기를 옮길 때</h3><p>새 환경에서 <strong>공유 코드</strong>를 입력하거나 JSON 파일을 불러오면 진행을 옮길 수 있습니다. 사이트 데이터 삭제·시크릿 모드·게임 주소 변경 시에는 기존 브라우저 저장을 찾지 못할 수 있습니다.</p></section><section class="save-guide-section save-guide-warning"><h3>공유 전에 꼭 알아두세요</h3><ul><li>공유 코드는 <strong>생성한 순간의 저장 복사본</strong>이며 이후 진행과 자동으로 동기화되지 않습니다.</li><li>코드를 아는 사람은 그 저장 데이터를 불러올 수 있으므로 필요한 사람에게만 전달하세요.</li><li>공유 코드나 파일을 불러오면 현재 로컬 진행이 교체되며, 적용 직전에 복구용 내부 백업이 생성됩니다.</li></ul></section><p class="save-guide-footnote">가장 안전한 보관은 브라우저 자동 저장 + 공유 코드 또는 JSON 파일 백업을 함께 사용하는 것입니다.</p><div class="save-dialog-actions"><button data-manage class="save-guide-secondary">세이브 관리 열기</button><button data-confirm class="save-guide-primary">확인</button></div>`;
 ...
 function addWarning(anchor){const b=document.createElement('button');b.className='save-storage-note';b.textContent='💾 자동 저장 · 공유 코드 · 백업 안내';b.onclick=g.showSaveWarning;anchor.after(b);}
