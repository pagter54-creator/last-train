...
(()=>{
 ...
 const C=window.RUN_CHECKPOINT_CONFIG={key:'lastTrainRunCheckpoint',warningKey:'lastTrainSaveGuideV3Acknowledged',legacyWarningKey:'lastTrainSaveWarningAcknowledged',saveGuideVersion:3,version:1,savedMs:1000};
 const LOCAL_STAMP_KEY='lastTrainLocalSaveStampV1',LOCAL_VERSION_KEY='lastTrainLocalSaveVersionV1';
 const touchLocalSave=()=>{try{localStorage.setItem(LOCAL_STAMP_KEY,new Date().toISOString());localStorage.setItem(LOCAL_VERSION_KEY,window.SAVE_FORMAT?.gameVersion||window.GAME_DATA?.version||'1.0.2');}catch{}};
 ...
 function commit(extra={}){
  ...
  try{const checked=valid(p);if(!checked)throw Error('Invalid node checkpoint');const json=JSON.stringify(checked);localStorage.setItem(C.key,json);touchLocalSave();status('저장됨');}
 ...
 g.saveStationCheckpoint09=function(){if(restoring||this.mode!=='station'||!this.stationOffers)return;const s=this.state;
  ...
  try{const checked=valid(p);if(!checked)throw Error('Invalid station checkpoint');localStorage.setItem(C.key,JSON.stringify(checked));touchLocalSave();current=`${s.runId}:${s.loop09||1}:${s.actId}:${s.stageIndex}`;pending=null;}catch(error){status('정비소 저장 실패 · 저장 공간을 확인하세요.',true);console.warn(error);}
 };
 ...
 function remove(){try{localStorage.removeItem(C.key);touchLocalSave();pending=null;current=null;return true;}catch{status('체크포인트를 삭제하지 못했습니다. 브라우저 저장 설정을 확인해 주세요.',true);return false;}}
 ...
 g.showSaveWarning=()=>{
  ...
  el.innerHTML=`<div class="save-guide-heading"><small>LOCAL · CLOUD · BACKUP</small><h2>저장 및 백업 안내</h2><p>LAST RAIL은 <strong>현재 브라우저에 자동 저장</strong>하며, 계정으로 로그인하면 같은 저장 데이터를 <strong>클라우드에도 자동 동기화</strong>할 수 있습니다.</p></div><div class="save-guide-grid"><article><b>로컬 자동 저장</b><p>영구 성장과 설정은 자동 저장됩니다. 진행 중인 런은 가장 최근 <strong>스테이지 시작 체크포인트</strong>에서 이어지며, 전투 도중의 순간 상태는 저장하지 않습니다.</p></article><article><b>계정 클라우드 저장</b><p>세이브 관리에서 이메일 계정으로 로그인하면 계정당 하나의 클라우드 세이브를 사용합니다. 동기화가 연결된 뒤에는 로컬 변경을 자동으로 서버에 반영합니다.</p></article><article><b>JSON 비상 백업</b><p>클라우드와 별개로 JSON 파일 내보내기·불러오기를 계속 지원합니다. 중요한 진행이나 수동 보관이 필요할 때 사용할 수 있습니다.</p></article></div><section class="save-guide-section"><h3>로그인할 때 저장 데이터 비교</h3><p>로컬 저장과 클라우드 저장이 모두 있으면 자동 동기화를 먼저 멈추고 <strong>저장 일자 · 저장 버전 · 플레이 시간 · 보유 고대 잔해 · 누적 사용 고대 잔해</strong>를 나란히 보여줍니다. 플레이어가 사용할 저장을 직접 선택한 뒤에만 자동 동기화가 시작됩니다.</p></section><section class="save-guide-section save-guide-warning"><h3>데이터를 지키는 가장 안전한 방법</h3><ul><li>평소에는 로컬 자동 저장과 계정 클라우드 동기화를 함께 사용할 수 있습니다.</li><li>브라우저·기기 변경 시에는 먼저 로그인한 뒤 기존 클라우드 세이브를 불러오세요.</li><li>큰 업데이트 전이나 별도 보관이 필요하면 <strong>JSON 저장 파일 내보내기</strong>를 추가로 사용하세요.</li></ul></section><p class="save-guide-footnote">클라우드 저장은 인터넷 연결이 필요합니다. 연결 실패 시에도 기존 로컬 자동 저장은 계속 유지됩니다.</p><div class="save-dialog-actions"><button data-manage class="save-guide-secondary">세이브 관리 열기</button><button data-confirm class="save-guide-primary">확인</button></div>`;
  document.body.append(el);
  const close=()=>{el.close();el.remove();};
  el.querySelector('[data-confirm]').onclick=()=>{acknowledgeSaveGuide();close();};
  el.querySelector('[data-manage]').onclick=()=>{acknowledgeSaveGuide();close();setTimeout(()=>{if(typeof g.showSaveManager==='function')g.showSaveManager();else g.toast?.('메인 메뉴의 세이브 관리에서 저장을 관리할 수 있습니다.');},0);};
 ...
 function addWarning(anchor){const b=document.createElement('button');b.className='save-storage-note';b.textContent='💾 로컬·클라우드 저장 및 백업 안내';b.onclick=g.showSaveWarning;anchor.after(b);}
