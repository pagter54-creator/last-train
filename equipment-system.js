(()=>{
 ...
 const upgrade=g.upgradeEquipment.bind(g);g.upgradeEquipment=function(id,choice){const eq=this.findEquipment(id);if(eq?.kind!=='module')return upgrade(id,choice);const plan=this.moduleUpgradePlan(eq);if(!plan)return false;if(plan.branch&&this.mode!=='station'){this.toast('모듈 분기 강화는 정비 스테이션에서만 가능합니다.');return false;}if(plan.branch&&!B.moduleUpgrade.branches[choice])return false;if(plan.branch&&choice==='specialized'&&(D.MODULES[eq.type]?.range||0)===0)return false;if(this.state.scrap<plan.cost){this.toast('고철이 부족합니다.');return false;}this.state.scrap-=plan.cost;eq.investedScrap=(eq.investedScrap||0)+plan.cost;eq.level=plan.level;if(plan.branch)eq.model=choice;this.rebalancePower();this.renderAll();this.playSound('upgrade');return true;};
 ...
 const prepare=g.prepareActShop.bind(g);g.prepareActShop=function(){prepare();const o=this.stationOffers;if(o.reformReady)return;o.reformReady=true;
  // 1.0.1 balance: the first module slot strongly prefers a module the train is not already using.
  const firstModule=o.gear.findIndex(x=>x.kind==='module'),mountedModules=new Set(this.state.cars.flatMap(c=>c.equipment).filter(e=>e.kind==='module').flatMap(e=>[e.type,e.aux?.type].filter(Boolean)));
  if(firstModule>=0&&mountedModules.has(o.gear[firstModule].id)&&Math.random()<.75){const shown=new Set(o.gear.filter(x=>x.kind==='module').map(x=>x.id)),pool=Object.entries(D.MODULES).filter(([id])=>(this.runContentUnlocked?.('modules',id)??true)&&!mountedModules.has(id)&&!shown.has(id));if(pool.length){const [id,d]=pool[Math.floor(Math.random()*pool.length)];o.gear[firstModule]={id,kind:'module',d};}else{const swap=o.gear.findIndex((x,i)=>i>firstModule&&x.kind==='module'&&!mountedModules.has(x.id));if(swap>=0)[o.gear[firstModule],o.gear[swap]]=[o.gear[swap],o.gear[firstModule]];}}
  const turrets=this.state.cars.flatMap(c=>c.equipment).filter(e=>e.kind==='turret'),mean=turrets.reduce((n,e)=>n+(e.level||1),0)/Math.max(1,turrets.length),band=C.shop.bands.filter(b=>mean>=b.at).at(-1)||C.shop.bands[0];let index=0;
  for(const offer of o.gear){const eq={id:crypto.randomUUID(),type:offer.id,kind:offer.kind,level:1,heat:0,cooldown:0,branch:false,weaponBranches:{},investedScrap:0};
   const escalation=this.escalationRules09?.();
   if(offer.kind==='turret'&&(index++>0||escalation?.shopTier>0)&&Math.random()<(escalation?.advancedChance??C.shop.chance)){let roll=Math.random();eq.level=band.levels.at(-1);for(let i=0;i<band.levels.length;i++){roll-=band.weights[i];if(roll<=0){eq.level=band.levels[i];break;}}eq.level=Math.min(C.shop.maxLevel,eq.level+(escalation?.shopTier||0));for(const[tier,choices]of Object.entries(W.tiers))if(eq.level>=Number(tier)){const ids=Object.keys(choices);eq.weaponBranches[tier]=ids[Math.floor(Math.random()*ids.length)];}for(let l=2;l<=eq.level;l++)eq.investedScrap+=W.costs[l];}
    if(offer.kind==='module'&&D.MODULES[eq.type].upgradeable!==false&&escalation?.shopTier>0&&Math.random()<escalation.advancedChance){eq.level=Math.min(4,1+escalation.shopTier);if(eq.level>=3)eq.model='standard';for(let l=2;l<=eq.level;l++)eq.investedScrap+=C.moduleCosts[l];}
