(()=>{
 ...
 const cost=g.upgradeCost.bind(g);g.upgradeCost=eq=>eq.kind==='module'?(D.MODULES[eq.type].upgradeable===false||eq.level>=5?null:C.moduleCosts[(eq.level||1)+1]):cost(eq);
 const upgrade=g.upgradeEquipment.bind(g);g.upgradeEquipment=function(id,choice){const eq=this.findEquipment(id);if(eq?.kind!=='module')return upgrade(id,choice);const price=this.upgradeCost(eq);if(price===null)return false;if(this.state.scrap<price){this.toast('고철이 부족합니다.');return false;}this.state.scrap-=price;eq.investedScrap=(eq.investedScrap||0)+price;eq.level=(eq.level||1)+1;this.rebalancePower();this.renderAll();this.playSound('upgrade');return true;};
 const seconds=g.equipmentUpgradeSeconds.bind(g);g.equipmentUpgradeSeconds=eq=>eq.kind==='module'?0:seconds(eq);
 g.attachAux=function(hostId,sourceId){if(this.mode!=='station')return false;const host=this.findEquipment(hostId),source=this.findEquipment(sourceId);if(!host||!source||host===source||host.kind!=='module'||source.kind!=='module'||host.level<C.auxUnlock||host.aux||source.aux||D.MODULES[host.type].upgradeable===false)return false;const car=this.state.cars.find(c=>c.equipment.includes(source));car.equipment.splice(car.equipment.indexOf(source),1);host.aux=source;this.rebalancePower();this.renderAll();this.playSound('equip');return true;};
