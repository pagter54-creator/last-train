...
(()=>{
 ...
 F.validateSave=function(input){
  ...
  const m=s.metaProgress;for(const k of ['relics','spent','totalEarned','clears','act2Clears','maxPoints','playSeconds'])m[k]=Math.max(0,n(m[k]));m.schema=C.version;m.upgrades=levels({...s.upgrades,...(s.captainUpgrades||{})},notes,m);m.discovered=obj(m.discovered)?m.discovered:{turrets:[],enemies:[]};
