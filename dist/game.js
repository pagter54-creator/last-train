(() => {
  ...
  class Game {
    ...
    stageCurve() { return D.STAGE_CURVE[Math.min(this.state.stageIndex, D.STAGE_CURVE.length - 1)]; }
    ...
    spawnEnemy(forceId) {
      const stage = this.state.stageIndex + 1;
      let pool = registries.enemies.entries().filter(([, e]) => e.fromStage <= stage);
      if (this.state.battle.allEnemies) pool = registries.enemies.entries();
    ...
    updateCrew(dt) {
      ...
      for (const c of s.crew) {
        ...
        } else if (s.doctrine && car.hp < car.maxHp * B.battle.doctrineRepairThreshold && this.effectiveStat(c,'repair') >= B.battle.doctrineMinRepairStat) {
    ...
    turretStats(eq, carIndex, operator) {
      const t = D.TURRETS[eq.type];
      const bonusPower = Math.max(0, this.state.cars[carIndex].power - B.train.baseCarPower);
    ...
    damageEnemy(target, amount, pierce = 0) {
      ...
      if (target.hp <= 0) {
        target.hp = 0;
        if ('destroyed' in target) { target.destroyed = true; this.log(`BEHEMOTH ${target.name} 파괴`, 'hot'); }
    ...
    updateTitan(dt) {
      const s = this.state;
      const stage = Math.min(s.stageIndex + 1, D.STAGE_CURVE.length);
      let titan = B.titan.speedBase + (stage - 1) * B.titan.speedPerStage;
      let train = B.train.speedByPower[s.cars[0].power] || B.train.speedByPower[B.train.enginePower.min];
    ...
    battleClear() {
      ...
      const stage = s.stageIndex + 1, elite = s.battle.elite;
